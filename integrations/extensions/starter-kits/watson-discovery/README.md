...
- Use either method listed in [Configuring Your Actions Skill to use an Extension](https://github.com/watson-developer-cloud/assistant-toolkit/blob/master/integrations/extensions/README.md#configuring-your-actions-skill-to-use-an-extension) to configure the actions you uploaded to invoke the custom extension you built.
   ...
   - In the second step of the "Search" action, set the following parameter values:
      ...
      - `count` = an expression with the value `3`
      - `passages.characters` = an expression with the value `250`
      - `passages.enabled` = an expression with the value `true`
      - `passages.fields` = an expression with the value `["text"]`
      - `passages.find_answers` = an expression with the value `true`
      - `table_results.enabled` = an expression with the value `false`
      - `return` = an expression with the value `["title","metadata.source.url"]`
      - `natural_language_query` = the `query_text` session variable
   - In the first step of the "Search within the mining topic" action, do the same thing plus the following:
...
You can also go to the "Preview" tab of Watson Assistant tooling and see what the results look like without the special features in `index.html`.  Here is an example of what the results might look like in this case:
...
As noted above, "No action matches" calls out to "Search" in "Created by you".  If you look at that action, you will see that the first step is to check if the `application_topic` matches `mining` -- this is an example of a specialized customization that we explain in more detail later (ignore it for now).  The second step calls the Watson Discovery search extension.  The third step provides an error message if the search failed.  The fourth calls out to the "Show search results" action.

The "Show search results" action calls the "Show search result" action up to three times depending on how many search results were received.  The "Show search result" action first checks to see if `user_defined_responses_enabled` is true.  This session variable is initialized to `false` (under "variables"/"set by you" within the Actions page).  However, it is set to `true` in `index.html` (discussed below).  So when this action is called from `index.html`, it produces one behavior (see below) and when it is called directly from the Watson Assistant preview or a simpler web chat configuration, it produces another.  Specifically, when "user_defined_responses_enabled" is false, "Show search result" continues on to the next two steps, which just renders the search result directly using simple HTML.  The second step shows a full response with a concise answer and a passage: it presents the answer first and then says "For more context on this answer see:" and then the title with a link to the page and the full passage from which the answer was extracted.  This is a bit cumbersome and not really a recommended way to show search results, but it was done this way in this starter kit to illustrate that you can take whatever information you want from the Discovery search API and organize it any way you want in a response.
