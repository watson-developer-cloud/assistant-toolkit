...
You can use this starter kit to configure language models with Watson Assistant for conversational search. This starter kit shows two ways to do this. The first is an example of [how the IBM watsonx tech preview language model and Watson Discovery search can be used to provide a personalized response to a customer](#example-1-connect-your-assistant-to-a-watsonx-language-model-via-custom-extensions). A second example shows how to use [semantic search output as input to OpenAI](#example-2-connect-your-assistant-to-hugging-face-milvus-and-openai-via-custom-extensions).

The [prerequisites](#prerequisites) are the same for both examples.
...
Before you upload the sample action for this starter kit, you first need to configure two custom extensions: [watsonx tech preview](../language-model-watsonx-tech-preview/README.md) and [Watson Discovery](../watson-discovery).
...
1. In **Authentication**, choose **No authentication** and enter values for your Milvus server variables (i.e., ip-address:port) to create a valid generated URL for requests. However, if you configured Milvus to require basic authentication, then choose **Basic auth** and enter your Milvus credentials.
