...
Click create queue and specify a name for the new queue. You can read [queue options](https://help.mypurecloud.com/articles/create-queues) for more information.
...
### Step 7: Configure OAuth for the Interaction Widget
...
### Step 8: Configure the Interaction Widget
...
`clientID`: generated when you created the OAuth credentials to be used for the implicit token login (see previous step).

`environment`: based on the location of your Genesys account. The environment shown works for North American accounts.

`conversationID`: use the default shown here. This is generated by Genesys at runtime.
...
On this page you will need to enable voice transcriptions and `Low Latency Transcription`. You can read the [speech setup documentation](https://help.mypurecloud.com/articles/speech-and-text-analytics) for more information.
...
This sample relies on the `string-similarity` algorithm to do the verbatim check. Read the  [string-similarity](https://github.com/aceakash/string-similarity) open source page for more information.
